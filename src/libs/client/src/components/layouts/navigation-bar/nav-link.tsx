...
const navLink = cva(
  "block h-full w-full border-b-2 border-transparent text-center font-light hover:text-selected",
  {
    variants: {
      device: {
...
export default function NavLink({
  ...
  return (
    <span {...props}>
      <Link href={routeObj.path} className={cn(navLink({ device }), className)}>
        {routeObj.name}
      </Link>
    </span>
  );
